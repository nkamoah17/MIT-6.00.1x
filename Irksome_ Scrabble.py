...
def getWordScore(word, n):
    ...
    score = 0   
    for i in word:
      added.append(SCRABBLE_LETTER_VALUES.get(i))
      k = sum(added)
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        displayHand(hand)
        # Ask user for input
        d = input('Enter word, or a "." to indicate that you are finished: ')
        ...
        # Otherwise (the input is not a single period):
        elif isValidWord(d, hand, wordList) is False:
            ...
            totalScore += getWordScore(d, n)
            print(str(d)  + " earned " +str(getWordScore(d, n)) +" points. " + "Total : " + str(totalScore) + " points")
            
                # Update the hand 
            hand = updateHand(hand, d)
